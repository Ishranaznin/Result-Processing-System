...
$status=1;
$sql_check = "SELECT COUNT(*) AS count FROM tblsubjectcombination WHERE ClassId = :class AND SubjectId = :subject";
$query_check = $dbh->prepare($sql_check);
$query_check->bindParam(':class', $class, PDO::PARAM_STR);
$query_check->bindParam(':subject', $subject, PDO::PARAM_STR);
$query_check->execute();
$row = $query_check->fetch(PDO::FETCH_ASSOC);
$combination_count = $row['count'];
if($combination_count > 0) {
        // Combination already exists, so discard the insertion
        $error = "Combination already exists in the table.";
    } else {
...
<div class="form-group">
    ...
    foreach($results as $result)
    {   ?>
    <option value="<?php echo htmlentities($result->id); ?>"><?php echo htmlentities($result->SubjectName . ' - ' . $result->SubjectCode); ?></option>
    <?php }} ?>
     </select>
    </div>
</div>
