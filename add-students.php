...
if(strlen($_SESSION['alogin'])=="")
    ...
    $status=1;
    $sql="INSERT INTO  tblstudents(Student_ID,StudentName,Roll,Gender,Science_Business_group,ClassId,Optional_Subject,Compulsory_Subject) VALUES(:studentid,:studentname,:roolid,:gender,:science_business_group,:classid,:optional_subject,:compulsory_subject)";
    ...
    <div class="col-sm-10">
    <input type="number" name="studentid" class="form-control" id="studentid" maxlength="5" required="required" autocomplete="off">
    </div>
    </div>

    <!-- <div class="form-group">
    <label for="default" class="col-sm-2 control-label">Class Rank</label>
    <div class="col-sm-10">
    <input type="number" name="rank" class="form-control" id="rank" maxlength="5" required="required" autocomplete="off">
    </div>
    </div> -->
...
<?php }} ?>
    ...
    <div class="form-group">
        <label for="default" class="col-sm-2 control-label">Compusory Subject</label>
        <div class="col-sm-10">
        <select name="compulsory_subject" class="form-control" id="default" required="required">
        <option value="">Select Compulsory Subject</option>
        <?php 
        $sql = "SELECT DISTINCT SubjectName FROM tblsubjects WHERE SubjectName NOT IN ('Bangla', 'English', 'Information & Communication Technology','Physics','Chemistry','Business Organization and Management','Accounting')";
        $query = $dbh->prepare($sql);
        $query->execute();
        $results=$query->fetchAll(PDO::FETCH_OBJ);
        if($query->rowCount() > 0)
    ...
    <div class="form-group">
        <label for="default" class="col-sm-2 control-label">Optional Subject</label>
        <div class="col-sm-10">
        <select name="optional_subject" class="form-control" id="default" required="required">
        <option value="">Select OptionalSubject</option>
        <?php 
        $sql = "SELECT DISTINCT SubjectName FROM tblsubjects WHERE SubjectName NOT IN ('Bangla', 'English', 'Information & Communication Technology','Physics','Chemistry','Business Organization and Management','Accounting')";
        $query = $dbh->prepare($sql);
        $query->execute();
